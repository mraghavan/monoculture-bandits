...
def run_experiment(n_agents, n_arms, n_rounds, n_simulations, n_0, output_file):
    ...
    print(f"--- Starting Experiment: n={n_agents}, t={n_rounds} ---")

    for i in range(n_simulations):
        try:
            print(f"  Running simulation {i+1}/{n_simulations} for n={n_agents}, t={n_rounds}...")
            ...
            print(f"Error in simulation {i+1}: {e}", file=sys.stderr)


    print(f"\n--- Final Averages for n={n_agents}, t={n_rounds} after {n_simulations} runs ---")
    with open(output_file, 'a') as f:
        ...
        for setting in sorted(total_regrets.keys()):
            ...
            line = f"{setting}: Regret={avg_regret:.2f}, Misclassified={avg_misclassified:.2f}"
            print(line)
            f.write(line + "\n")
        print("\n")
        f.write("\n")
...
def main():
    ...
    agent_counts = [10, 25, 50, 75, 90]
    round_counts = [100, 200]
    ...
    for n_agents in agent_counts:
        for n_rounds in round_counts:
            run_experiment(n_agents=n_agents, n_arms=n_arms, n_rounds=n_rounds,
